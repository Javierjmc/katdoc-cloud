'use client';
// components/DewormingSection.tsx
// Desparasitaciones (S40): sección propia con catálogo rápido.
// Se guardan en vaccinations con categoria='desparasitacion'.
...
import { DESPARASITANTES, type Vaccination } from '@/types';
...
export default function DewormingSection({ patientId }: { patientId: string }) {
  const { vaccinations, loading, refetch } = useVaccinations(patientId, 'desparasitacion');
  ...
  const handleDelete = async () => {
    ...
    else { toast('Desparasitación eliminada', 'success'); setToDelete(null); refetch(); }
  };
    ...
    <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-200 dark:border-surface-700 shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-surface-100 dark:border-surface-800">
        <button onClick={() => setOpen(o => !o)} className="flex items-center gap-2 text-left group" aria-expanded={open}>
          <h3 className="text-sm font-black text-surface-700 dark:text-surface-200 group-hover:text-brand-600">🐛 Desparasitaciones</h3>
      ...
      <div className="p-4 space-y-2">
        {loading ? (
          ...
          <EmptyState icon="🐛" title="Sin desparasitaciones" subtitle="Registra la próxima desparasitación del paciente."
            action={<Button size="sm" onClick={openCreate}>Registrar desparasitación</Button>} />
        ) : (
          vaccinations.map(v => (
            <div key={v.id} className="flex items-center gap-3 p-3 rounded-xl border border-surface-200 dark:border-surface-700 hover:border-brand-400 transition-colors">
              <div className="flex-1 min-w-0">
                ...
                {v.marca && <p className="text-xs text-surface-400 dark:text-surface-500 mt-0.5">{v.marca}</p>}
              </div>
      ...
      {editor && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => !saving && setEditor(null)}>
          <div className="w-full max-w-lg rounded-3xl bg-white dark:bg-surface-800 border border-surface-200 dark:border-surface-700 shadow-2xl p-6 space-y-3" onClick={e => e.stopPropagation()}>
            <h3 className="text-base font-bold text-surface-800 dark:text-white">{editor.mode === 'create' ? 'Registrar desparasitación' : 'Editar desparasitación'}</h3>

            <Field label="Desparasitante" required>
              <Input value={editor.data.vacuna} onChange={e => setField('vacuna', e.target.value)}
                placeholder="Ej: Drontal Plus" list="desparasitantes-sugeridos" />
              <datalist id="desparasitantes-sugeridos">
                {DESPARASITANTES.map(d => <option key={d} value={d} />)}
              </datalist>
            </Field>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Marca">
                <Input value={editor.data.marca ?? ''} onChange={e => setField('marca', e.target.value)} placeholder="Ej: Bayer" />
              </Field>
              <Field label="Dosis">
